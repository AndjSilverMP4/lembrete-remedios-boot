...
def encontrar_nome_proximo(nome_digitado, nomes_validos):
    ...
    correspondencias = difflib.get_close_matches(nome_digitado, [n.lower() for n in nomes_validos], n=1, cutoff=0.6)
    if correspondencias:
...
def responder():
    ...
    if "tomei" in texto or texto in ["sim", "claro", "foi", "confirmado", "já tomei"]:
        ...
        historico["pendencias"] = [
            p for p in historico.get("pendencias", [])
            if not (p["remedio"].lower() == nome_confirmado.lower() and p["data"] == hoje)
        ]
        salvar_json(HISTORICO_ARQUIVO, historico)
    ...
    if "falta" in texto or "pendente" in texto or "quais" in texto:
        ...
        for r in remedios:
            for h in r["horarios"]:
                confirmado = any(
                    c["remedio"].lower() == r["nome"].lower() and c["data"] == hoje and c["hora"] == h["hora"]
                    for c in historico.get("confirmacoes", [])
                )
                if not confirmado:
        ...
        if pendentes:
            resposta.message(f"📋 Ainda falta tomar:\n" + "\n".join(f"🔔 {p}" for p in pendentes))
        else:
